    ...
    for f in file_ls:
        ...
        if len(dets)>0:
            ...
            for k, d in enumerate(dets):
                ...
                """
                for i in range(0,68):
                    if i==44 or i==37 or i==30:
                        center=(shape.part(i).x,shape.part(i).y)
                        cv2.circle(img,center,1,(0,0,255),-1)
                        cv2.putText(img,str(i),center,cv2.FONT_HERSHEY_PLAIN,1,(255,255,0))
                """
                ...
                a=np.array((shape.part(43).x,shape.part(43).y))-np.array((shape.part(38).x,shape.part(38).y))
                b=np.array((shape.part(30).x,shape.part(30).y))-np.array((shape.part(38).x,shape.part(38).y))
                c=np.array((shape.part(30).x,shape.part(30).y))-np.array((shape.part(43).x,shape.part(43).y))
                if np.dot(a,b)*np.dot(a,c)>0 or shape.part(27).x<img.shape[1]//2-20 or shape.part(27).x>img.shape[1]//2+20 or shape.part(27).y<img.shape[0]//2-20 or shape.part(27).y>img.shape[0]//2+20:
                    dir_path="trash/"
                    if os.path.isdir(dir_path)==False:
                        os.mkdir(dir_path)
                    PIL_im.save(dir_path+re.sub(".jpg","",f.split('/')[-1])+datetime.now().strftime("%Y%m%d-%H%M%S")+".jpg")
                    break
                dir_path="detected"+str(number)+"/"
                if os.path.isdir(dir_path)==False:
                    os.mkdir(dir_path)
                PIL_im.save(dir_path+re.sub(".jpg","",f.split('/')[-1])+datetime.now().strftime("%Y%m%d-%H%M%S")+".jpg")
...
jobs=[]
for i in range(core):
    job=Process(target=face_crop,args=(file_list[i*file_count:(i+1)*file_count],i)) 
    jobs.append(job)
    job.start()
